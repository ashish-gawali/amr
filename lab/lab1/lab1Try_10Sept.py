...
class shifty:
	...
	def init_irSensor(self):
		def ir_callback(data):
			rospy.loginfo("detected %f, number= %f", data.range, self.iterationsToRotateFor)
			if(data.range < 7 and self.iterationsToRotateFor == 0):
				self.iterationsToRotateFor = 30
				var = random.randint(0,1)*2 -1
				self.angular_velocity = var			

		rospy.Subscriber("/range/fl", Range, ir_callback)
